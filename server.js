...
app.get("/api/users", async (req, res) => {
  try {
    ...
    limit = Math.max(1, Math.min(100, limit)); // 1 <= limit <= 100
    ...
    // Query database với Promise.all (song song)
...
app.post("/api/users", async (req, res) => {
  try {
    ...
    age = parseInt(age); // Chuyển về số nguyên
...
app.put("/api/users/:id", async (req, res) => {
  try {
    ...
    // Tạo object chỉ chứa các trường được truyền vào
    const updateData = {};
